...
public class HealthBarRenderer {
	...
	public static ItemStack getIcon(LivingEntity entity, boolean boss) {
		...
		MobType attr = entity.getMobType();
		if (attr == MobType.ARTHROPOD) {
			return new ItemStack(Items.SPIDER_EYE);
		} else if (attr == MobType.UNDEAD) {
	...
	public void onRenderWorldLast(RenderLevelLastEvent event) {
		...
		if (NeatConfig.showOnlyFocused) {
			...
			if (focused != null && focused instanceof LivingEntity && focused.isAlive()) {
				renderHealthBar((LivingEntity) focused, mc, poseStack, partialTicks, renderInfo, cameraEntity);
			}
		} else {
			Vec3 cameraPos = renderInfo.getPosition();
			final Frustum clippingHelper = new Frustum(poseStack.last().pose(), event.getProjectionMatrix());
			...
			if (client != null) {
				for (Entity entity : client.entitiesForRendering()) {
					if (entity != null && entity instanceof LivingEntity && entity != cameraEntity && entity.isAlive() && Iterables.isEmpty(entity.getIndirectPassengers()) && entity.shouldRender(cameraPos.x(), cameraPos.y(), cameraPos.z()) && (entity.noCulling || clippingHelper.isVisible(entity.getBoundingBox()))) {
						renderHealthBar((LivingEntity) entity, mc, poseStack, partialTicks, renderInfo, cameraEntity);
					}
				}
			}
		}
	...
	public void renderHealthBar(LivingEntity passedEntity, Minecraft mc, PoseStack poseStack, float partialTicks, Camera renderInfo, Entity viewPoint) {
		...
		while (!ridingStack.isEmpty()) {
			...
			processing:
			{
				...
				Vec3 renderPos = renderManager.camera.getPosition();
				...
				renderEntity(mc, poseStack, buffer, renderInfo, entity, light, icon, boss);
				poseStack.popPose();
	...
	private void renderEntity(Minecraft mc, PoseStack poseStack, MultiBufferSource.BufferSource buffer, Camera renderInfo, LivingEntity entity, int light, ItemStack icon, boolean boss) {
